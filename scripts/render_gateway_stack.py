...
def render_docker_compose(config: Dict[str, Any]) -> str:
    gw = config.get("gateway", {})
...
services:
  ...
  litellm:
    ...
    environment:
      DATABASE_URL: postgresql://${{DB_USER:-postgres}}:${{DB_PASSWORD}}@${{DB_HOST:-postgres}}:${{DB_PORT:-5432}}/${{DB_NAME:-sooniverse}}?schema=sooniverse
...
def render(config: Dict[str, Any]) -> None:
    ...
    COMPOSE_PATH.write_text(render_docker_compose(config), encoding="utf-8", newline="\n")
